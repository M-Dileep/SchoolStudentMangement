...
//    }
    @GetMapping("/GetScreens/{user}")
    public ResponseEntity<Permission> getPermissionByUser (@PathVariable(value="user") String permissionUser) {
      Permission permission= entityManager.createQuery("SELECT p FROM Permission p WHERE p.user = :user", Permission.class)
                                           .setParameter("user", permissionUser)
                                           .getSingleResult();
        if (permission  == null) {
            return ResponseEntity.notFound().build();
        }
